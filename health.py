...
data1 = pd.read_csv('cleaned_health_dataset.csv') 
data2 = pd.read_csv('country_metadata.csv')       
...
master_df = pd.merge(data1, data2, on='country_code', how='left')
...
def get_dashboard_data(country_code: str):
    ...
    country_df = master_df[master_df['country_code'] == country_code.upper()]
